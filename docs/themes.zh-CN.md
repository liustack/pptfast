...
每个内置主题默认对每个页型都开放全部已注册版式，只有五家的封面被锁到了设计板的那一构造。每个版式都会按主题的实际背景色自适应取色，所以全集在任何主题下都保持可读。收窄集合是主题作者的主动选择。`stage`、`lecture`、`swiss`、`memo`、`playbill` 在 `layouts.cover` 上动用了它，封面就是板上那一页，不是池里换了配色的邻居。

其余主题收窄的是偏好：写明自己偏向哪几个封面，同一份 deck、同一个 seed，换个主题通常会抽到不一样的封面。偏好不是锁定，封面必须是某一个时，在这一页上写 `slide.layout` 钉死。上面五家已经把池锁死，偏好和池是同一个 id。
