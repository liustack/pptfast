...
- **Every revision flows through the deck project's gates.** The single source of truth is `deck.spec.json` + `pages/*.json`, and every edit passes `validate`/`audit`. Preview stays read-only by design — shipped: `preview.html` overlays `audit` findings and offers in-page annotations that export as a `revision-request.json`, but that file is only ever a set of requests for an agent to route into `pages/*.json`, never a write path of its own (`docs/deck-projects.md`'s six-phase workflow). `pptfast serve` (same doc) is the natural extension the layered design predicted: a live version of the identical read-only page whose submit control POSTs that same `revision-request.json` straight to the deck directory instead of only downloading it — the gate the settled decision protects stays intact either way, since `pages/*.json` itself is still never written directly, only the request file an agent must still route through the normal edit path. A full in-preview editor is deliberately deferred either way.
