---
"@liustack/pptfast": minor
---

Fourteen new content-page faces for eight existing components. The IR, schema, and skill routing table do not change. A theme looks up its face from a render-side assignment table.

**Decks that pick one of the 41 assigned (theme, component) pairs look different.** Unassigned themes keep the current face, byte for byte. campaign's cycle face is hub-spoke (first listing wins). bloom is not assigned.

`icon_cards` gains column, badge, and outline-grid faces. `cycle` gains loop, hub-spoke, and petal-wheel. `numbered_cards` gains pills and hex cluster. `kpi_cards` gains donut trio and bubble row. `comparison` gains pill panels. `steps` gains an arrow band. `timeline` gains vertical nodes. `image_grid` gains numbered photos.
