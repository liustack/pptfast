...
The cover pool grows by six, and nine themes (plus bloom, which follows classroom) lock their cover to the face drawn on the design board.
...
`consulting` locks `verdict-index`. `classroom`, `enterprise`, and `vermilion` lock `band-title` (bloom follows classroom). `crayon` locks `header-band`. `runway` locks `paper-masthead`. `pulse` locks `horizon-wedge`. `arena` and `ember` lock `corner-wedge`. Other page types stay on the full set.
