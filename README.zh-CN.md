...
bullets 需要时会在各自档位基线之下收缩以适配空间，最低到 14px 地板，再触发溢出处理。对 `default`、`plain`、`divided` 三种要点样式而言，一条要点长到在这个地板字号下仍然放不下，会是一条硬校验错误——不同于（也宽于）上面按 pacing 分档的长度指导，因为它在渲染时会真的被省略号截掉文字，不只是显得啰嗦。`numbered` 与 `checklist` 两种样式可能在到达这条错误阈值之前就先被截断——这两种样式下真正兜底的是 `pptfast audit` 的「内容截断」检查，而不是 `validate`。`pptfast validate` 会报出每页实际生效的具体数值。
