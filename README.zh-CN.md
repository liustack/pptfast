...
自由绘制 SVG/HTML 再转 PPTX 的链路上限很高，但下限不稳定——弱模型（或强模型状态不好时）画出来的往往是版式错乱、脱离品牌规范、甚至无法阅读的产物。pptfast 用受控词汇取代自由绘制：一份语义化 IR（zod schema）、由 design tokens 驱动的 13 个内置风格、带 seed 多样性的 archetype/block 版式库，以及每个图形都保持可编辑的原生 DrawingML 输出——不是贴上去的一张图。
...
# → OK — 5 slides, style "consulting"
node dist/cli.js render examples/basic.json -o out/basic.pptx
# → wrote out/basic.pptx (5 slides, ~29 KB)
node dist/cli.js render examples/basic.json -o out/basic-tech.pptx --style tech
...
| 命令 | 作用 |
|---|---|
| `render <ir.json> -o <out.pptx> [--style <id>] [--tokens <file>]` | 校验并渲染成 `.pptx` |
| `validate <ir.json>` | 校验 IR，输出带页码的错误信息 |
| `schema [--tokens]` | 输出 IR 的 JSON Schema（或品牌 tokens 的 schema） |
| `styles [--json]` | 列出 13 个内置风格 |
...
运行 `node dist/cli.js schema` 获取完整 JSON Schema——让模型写 IR 之前先读它。一份 deck（`PptxIR`）包含 `version`（现为 `"3"`）、`filename`、`style`、`meta`、`assets`——均可省略、有默认值——外加可选的 `brand`，以及必填的有序 `slides` 列表。每张 slide 有一个 `type`（`cover`、`chapter`、`content`、`ending`），content 类型的 slide 还带一组带类型的 `blocks`（`bullets`、`kpi_cards`、`image`、`chart` 等）。`assets` 的形状是 `{ images: { [id]: { src, alt? } } }`，block 通过 `asset_id` 引用图片，同一张图可以在多页复用而不必重复内嵌。

## 风格

一个 style 由 theme（设计 tokens）、master（品牌母版元素）与亲和标签组成——以下是 13 个内置 style。替换 tokens（`--tokens`）即可为某个 style 重新配色。
...
不必分叉风格即可覆盖内置色板：写一份 tokens JSON（结构见 `pptfast schema --tokens`），按次渲染传入（`--tokens brand.json`），或固化在项目级 `pptfast.config.json` 里（自 cwd 向上查找，用 `pptfast init` 生成模板）。优先级：CLI flag > 配置文件 > IR。IR 自身也可以在 `style.tokens` 携带同样的覆盖，做到单文件自包含。

```json
{ "style": "consulting", "tokens": { "colors": { "primary": "#0B5FFF", "accent": "#FF6A00" } } }
```
...
- **v0.2**——封装该回路的 Claude Code plugin + skill（已落地）、design token 覆盖（`--tokens`）、`init`/自更新命令。
