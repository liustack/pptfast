...
| 命令 | 作用 |
|---|---|
| `render <ir.json> -o <out.pptx> [--theme <id>] [--tokens <file>]` | 校验并渲染成 `.pptx` |
...
不必分叉主题即可覆盖内置色板：写一份 tokens JSON（结构见 `pptfast schema --tokens`），按次渲染传入（`--tokens brand.json`），或固化在项目级 `pptfast.config.json` 里（自 cwd 向上查找，用 `pptfast init` 生成模板）。优先级：CLI flag > 配置文件 > IR。IR 自身也可以在 `theme.tokens` 携带同样的覆盖，做到单文件自包含。
