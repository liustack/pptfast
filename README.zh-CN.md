...
当某页省略 `layout` 时，pptfast 按五个确定性步骤自动选型：该页型的全部注册版式 → 组件必须能装进候选版式的 slot → 主题该页型的版式集合（默认全集，见上文「主题」）→ 场景的 `mode` 对一小撮适配该 mode 的 content 版式做 ×3 软加权，其余维持 ×1 底权（cover/chapter/ending 三个页型永不加权——它们的个性来自主题，不来自 mode）→ 按 seed 加权取样，若命中结果与紧邻的上一页版式相同则确定性地换成次优候选。显式 `layout` 恒优先，跳过以上全部步骤。
