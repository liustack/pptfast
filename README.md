...
Run `node dist/cli.js schema` for the full JSON Schema — feed it to a model before asking it to write IR. A deck (`PptxIR`) has `version` (currently `"3"`), `filename`, `style`, `meta`, and `assets` — all optional with sane defaults — plus an optional `brand` and a required ordered list of `slides`. Each slide has a `type` (`cover`, `chapter`, `content`, `ending`) and, for content slides, a list of typed `blocks` (`bullets`, `kpi_cards`, `image`, `chart`, …). `assets` is `{ images: { [id]: { src, alt? } } }` — blocks reference images by `asset_id`, so the same image can be reused across slides without duplication.

## Styles

A style is a theme (design tokens) plus a master (brand chrome) plus affinity tags — the 13 built-ins below. Swap tokens (`--tokens`) to re-color a style.
...
Override the built-in palette without forking a style: write a tokens JSON
...
override in `style.tokens` for fully self-contained decks.
