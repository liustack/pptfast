...
import type { BlockBox, BlockCtx, SvgBlock } from "./types"
 ...
 * padding convention when a block is dropped into a bento cell, which does
 * NOT happen for `steps` — see the "not exploded" note on the block dispatch
...
// Narrowest a horizontal card may get before this block switches to a
...
function renderBadge(cx: number, cy: number, n: number, ctx: BlockCtx): React.ReactElement {
  return (
    <>
...
function cardGeometry(block: StepsBlock, w: number) {
  const n = block.items.length
  const cardW = (w - GAP * (n - 1)) / n
  const contentW = cardW - PAD_X * 2
  const cardH = Math.max(
    ...block.items.map((item) => PAD_TOP + stepContentHeight(item, contentW) + PAD_BOTTOM),
  )
...
function verticalRowGeometry(block: StepsBlock, w: number) {
...
function renderVertical(block: StepsBlock, box: BlockBox, ctx: BlockCtx): React.ReactElement {
  const { contentW, rowH } = verticalRowGeometry(block, box.w)
...
export const steps: SvgBlock<StepsBlock> = {
  measure(block, w) {
    const n = block.items.length
    if (needsVerticalLayout(n, w)) {
      return n * verticalRowGeometry(block, w).rowH
    }
    return cardGeometry(block, w).cardH
  },
  render(block, box, ctx) {
    const n = block.items.length
    if (needsVerticalLayout(n, box.w)) {
      return renderVertical(block, box, ctx)
    }
    const { cardW, contentW, cardH } = cardGeometry(block, box.w)
    return (
      <g transform={`translate(${box.x},${box.y})`}>
        {block.items.map((item, i) => {
