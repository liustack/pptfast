...
type ComparisonBlock = Extract<Block, { type: "comparison" }>
 ...
 * Header titles: first column header is empty (row labels), rest are block.columns.
 */
function headerTitles(block: ComparisonBlock, labelHeader = ""): string[] {
  return [labelHeader, ...block.columns]
}
 ...
 * 不归一——可能是真实数据巧合，宁可保守。
 */
function dedupeLabelColumn(block: ComparisonBlock): {
  labelHeader: string
  ...
  const dup =
    ...
    block.rows.every(
      (r) =>
        r.cells.length === block.columns.length &&
        (r.cells[0] ?? "").trim() === r.label.trim(),
    )
  if (!dup) return { labelHeader: "", block }
  return {
    labelHeader: block.columns[0],
    block: {
      ...block,
      columns: block.columns.slice(1),
      rows: block.rows.map((r) => ({ ...r, cells: r.cells.slice(1) })),
    },
  }
}
 ...
 */
function columnTexts(block: ComparisonBlock, colIdx: number): string[] {
  if (colIdx === 0) {
    return block.rows.map((r) => r.label)
  }
  return block.rows.map((r) => r.cells[colIdx - 1] ?? "")
}
...
function computeColumns(
  ...
  const headers = headerTitles(block, labelHeader)
  const colCount = headers.length

  // Measure weight per column
  const weights: number[] = []
  for (let c = 0; c < colCount; c++) {
    const texts = [headers[c], ...columnTexts(block, c)]
...
export const comparison: SvgBlock<ComparisonBlock> = {
  measure(block) {
    return (block.rows.length + 1) * ROW
  },

  render(rawBlock, box, ctx) {
    const { labelHeader, block } = dedupeLabelColumn(rawBlock)
    const headers = headerTitles(block, labelHeader)
    const colCount = headers.length
    const { widths, offsets } = computeColumns(block, box.w, labelHeader)
    const borderColor = ctx.colors.border ?? ctx.colors.muted
    const totalRows = block.rows.length + 1 // header + data rows
    ...
    return (
      <g transform={`translate(${box.x},${box.y})`}>
        ...
        <line
          ...
          strokeWidth={2}
        />
        {Array.from({ length: block.rows.length - 1 }, (_, k) => (
          <line
            key={`sep-${k}`}
            x1={0}
            y1={(k + 2) * ROW}
