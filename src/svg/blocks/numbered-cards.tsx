...
type NumberedCardsBlock = Extract<Block, { type: "numbered_cards" }>
...
function grid(block: NumberedCardsBlock, w: number) {
...
function cellLayout(
  item: NumberedCardsBlock["items"][number],
  contentW: number,
) {
...
function rowHeights(block: NumberedCardsBlock, w: number, _ctx: BlockCtx) {
  const { cols, rows, contentW } = grid(block, w)
  const heights: number[] = []
  for (let r = 0; r < rows; r++) {
    const rowItems = block.items.slice(r * cols, (r + 1) * cols)
    heights.push(Math.max(...rowItems.map((it) => cellLayout(it, contentW).h)))
  }
  return heights
}

export const numberedCards: SvgBlock<NumberedCardsBlock> = {
  measure(block, w, ctx) {
    const heights = rowHeights(block, w, ctx)
    return heights.reduce((sum, h) => sum + h, 0) + (heights.length - 1) * ROW_GAP
  },
  render(block, box, ctx) {
    const { cols, contentW, cellW } = grid(block, box.w)
    const heights = rowHeights(block, box.w, ctx)
    return (
