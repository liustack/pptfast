...
describe("callout block", () => {
  const block = { type: "callout" as const, variant: "info" as const, text: "提示信息文本内容" }

  it("measures a positive height", () => {
    const h = callout.measure(block, 1120, ctx)
    expect(h).toBeGreaterThan(0)
  })

  it("renders a background rect with surface fill", () => {
    const { container } = svg(
      callout.render(block, { x: 80, y: 100, w: 1120 }, ctx),
    )
  ...
  it("renders a left bar rect with width=4 and warn variant uses #DC2626", () => {
    const warnBlock = { type: "callout" as const, variant: "warn" as const, text: "警告" }
    const { container } = svg(
      callout.render(warnBlock, { x: 0, y: 0, w: 800 }, ctx),
    )
    const rects = container.querySelectorAll("rect")
...
describe("callout card stroke (Task 5d)", () => {
  ...
  it("does not draw a stroke when ctx.colors.cardStroke is unset (every theme before this task)", () => {
    const { container } = svg(callout.render(block, { x: 80, y: 100, w: 1120 }, ctx))
  ...
  it("draws a 1px stroke in cardStroke's color when the token is set", () => {
    ...
    const { container } = svg(callout.render(block, { x: 80, y: 100, w: 1120 }, strokedCtx))
    const bgRect = container.querySelectorAll("rect")[0]
...
describe("callout block emphasis", () => {
