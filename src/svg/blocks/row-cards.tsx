...
type RowCardsBlock = Extract<Block, { type: "row_cards" }>
...
function cardLayout(item: RowCardsBlock["items"][number], w: number) {
...
export const rowCards: SvgBlock<RowCardsBlock> = {
  measure(block, w) {
    return (
      block.items.reduce((sum, item) => sum + cardLayout(item, w).cardH, 0) +
      (block.items.length - 1) * CARD_GAP
    )
  },
  render(block, box, ctx) {
    const layouts = block.items.map((item) => cardLayout(item, box.w))
    const measuredH =
    ...
    let visible = block.items.length
    ...
    return (
      <g transform={`translate(${box.x},${box.y})`}>
        {block.items.slice(0, visible).map((item, i) => {
