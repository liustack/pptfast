...
describe("comparison block", () => {
  ...
  it("renders no background fills at all (blends into any theme bg)", () => {
    const { container } = svg(
      comparison.render(block, { x: 80, y: 100, w: 1000 }, ctx),
    )
  ...
  it("sets the header off with a heavier text-colored rule instead of a fill", () => {
    const { container } = svg(
      comparison.render(block, { x: 0, y: 0, w: 1000 }, ctx),
    )
  ...
  it("closes the table with a light bottom rule", () => {
    ...
    const lines = Array.from(container.querySelectorAll("line"))
    const bottomY = String((block.rows.length + 1) * 44)
  ...
  it("renders correct number of text elements for headers and cells", () => {
    ...
    const minExpected = block.columns.length + block.rows.length * (block.columns.length + 1)
  ...
  it("renders separator lines between rows", () => {
    ...
    expect(lines.length).toBe(block.rows.length + 1)
  ...
  it("measure returns (rows + 1) * 44", () => {
    const h = comparison.measure(block, 1000, ctx)
    expect(h).toBe((block.rows.length + 1) * 44)
  })
  ...
  it("allocates wider column to longer text content", () => {
    ...
    const { container } = svg(
      comparison.render(wideBlock, { x: 0, y: 0, w: 1200 }, ctx),
    )
  ...
  it("wraps in a translated group", () => {
    const { container } = svg(
      comparison.render(block, { x: 120, y: 300, w: 800 }, ctx),
    )
  ...
  describe("shrink-before-truncate", () => {
    ...
    it("renders a moderately long cell in full at a reduced font size", () => {
      ...
      const { container } = svg(
        comparison.render(cellBlock, { x: 0, y: 0, w: 400 }, ctx),
      )
  ...
  it("falls back to muted color when border is not set", () => {
    const noBorderCtx: BlockCtx = {
      ...ctx,
      colors: { ...ctx.colors, border: undefined },
    }
    const { container } = svg(
      comparison.render(block, { x: 0, y: 0, w: 1000 }, noBorderCtx),
    )
    const line = container.querySelectorAll("line")[1]
...
describe("comparison 首列重复归一化（2026-07-10 无图矩阵真机病型：模型把 label 又抄进 cells[0]）", () => {
  it("全部行 cells[0]===label 且 cells 长度等于 columns 长度时：丢 cells[0]，columns[0] 移作标签列表头", () => {
    ...
    const { container } = render(
      <svg>{comparison.render(dupBlock, { x: 80, y: 200, w: 1120 }, ctx)}</svg>,
    )
  ...
  it("非全行命中（真实数据巧合）不归一", () => {
    ...
    const { container } = render(
      <svg>{comparison.render(okBlock, { x: 80, y: 200, w: 1120 }, ctx)}</svg>,
    )
    const texts = Array.from(container.querySelectorAll("text")).map((t) => t.textContent)
