...
type TimelineBlock = Extract<Block, { type: "timeline" }>
 ...
 */
function milestoneLayout(block: TimelineBlock, w: number) {
  const n = block.milestones.length
  const span = w - 2 * PAD
  const step = n > 1 ? span / (n - 1) : 0
  return block.milestones.map((m, i) => {
...
function verticalLayout(block: TimelineBlock, w: number) {
  const textW = Math.max(1, w - V_TEXT_X)
  return block.milestones.map((m) => {
...
function renderVertical(
  ...
  const rows = verticalLayout(block, box.w)
...
export const timeline: SvgBlock<TimelineBlock> = {
  measure(block, w, _ctx: BlockCtx) {
    if (block.layout === "vertical") {
      const rows = verticalLayout(block, w)
      const total = rows.reduce((sum, r) => sum + r.rowH + V_ROW_GAP, V_TOP_PAD)
      return total - V_ROW_GAP + BOTTOM_PAD
    }
    const rows = milestoneLayout(block, w)
    const maxBelow = rows.reduce((mx, r) => Math.max(mx, r.belowH), 48)
    return AXIS_Y + maxBelow + BOTTOM_PAD
  },
  render(block, box, ctx) {
    if (block.layout === "vertical") return renderVertical(block, box, ctx)
    const rows = milestoneLayout(block, box.w)
    return (
      <g transform={`translate(${box.x},${box.y})`}>
        <line
          x1={PAD}
