...
describe("quote block", () => {
  const blockNoAttr = { type: "quote" as const, text: "知识就是力量。" }
  const blockWithAttr = {
    type: "quote" as const,
    text: "知识就是力量。",
  ...
  it("measure with attribution is greater than without", () => {
    const hNoAttr = quote.measure(blockNoAttr, 1120, ctx)
    const hWithAttr = quote.measure(blockWithAttr, 1120, ctx)
  ...
  it("renders a translated group with correct transform", () => {
    const { container } = svg(
      quote.render(blockWithAttr, { x: 80, y: 200, w: 1120 }, ctx),
    )
  ...
  it("renders body text with text color and italic style", () => {
    const { container } = svg(
      quote.render(blockNoAttr, { x: 0, y: 0, w: 1120 }, ctx),
    )
  ...
  it("renders attribution line with muted color when present", () => {
    const { container } = svg(
      quote.render(blockWithAttr, { x: 0, y: 0, w: 1120 }, ctx),
    )
