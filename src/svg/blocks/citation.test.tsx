...
describe("citation block", () => {
  ...
  const block = { type: "citation" as const, sources }

  it("measure returns sources count times row height", () => {
    const ROW = 28
    expect(citation.measure(block, 1120, ctx)).toBe(sources.length * ROW)
  ...
  it("renders one <text> per source with sequential numbering", () => {
    const { container } = svg(
      citation.render(block, { x: 80, y: 500, w: 1120 }, ctx),
    )
  ...
  it("label text uses ctx.colors.text fill", () => {
    const { container } = svg(
      citation.render(block, { x: 0, y: 0, w: 1120 }, ctx),
    )
    const texts = container.querySelectorAll("text")
  ...
  it("shrinks the '[n] label' font-size when it would overflow 60% of the row width", () => {
    ...
    const { container } = svg(
      citation.render(narrowBlock, { x: 0, y: 0, w: 300 }, ctx),
    )
