...
describe("paragraph block", () => {
  const block = { type: "paragraph" as const, text: "这是一段用于测试换行与排版的中文段落文本，需要足够长以触发多行换行。" }

  it("measures a positive height that grows when width shrinks", () => {
    const wide = paragraph.measure(block, 1120, ctx)
    const narrow = paragraph.measure(block, 300, ctx)
  ...
  it("renders a translated group of text lines in the body font and text color", () => {
    const { container } = svg(
      paragraph.render(block, { x: 80, y: 264, w: 1120 }, ctx),
    )
...
describe("paragraph block emphasis", () => {
