...
function cardGeometry(block: IconCardsBlock, w: number) {
...
export const iconCards: SvgBlock<IconCardsBlock> = {
  measure(block, w) {
    const { rows, cardH } = cardGeometry(block, w)
    return rows * cardH + (rows - 1) * GAP
  },
  render(block, box, ctx) {
    const { cols, rows, cardW, contentW, cardH } = cardGeometry(block, box.w)
