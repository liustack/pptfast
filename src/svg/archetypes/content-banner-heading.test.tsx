...
describe("BannerHeadingContent", () => {
  ...
  it("不画自己的 y=648 源信息 hairline，footnote 落在其下方", () => {
    ...
    // The footnote now sits *on* the shared baseline (`FOOTNOTE_BASELINE_Y`,
    // brand-chrome.tsx) rather than 28px below the hairline this archetype
    // does not draw. At the old +28 it landed at 676 — under the footer
    // divider at 664, through the footer's own text row.
    expect(Number(footnote.getAttribute("y"))).toBe(FOOTNOTE_BASELINE_Y)
    expect(FOOTNOTE_BASELINE_Y).toBeLessThan(664)
  })
