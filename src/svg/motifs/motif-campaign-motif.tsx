...
/**
 * campaign-motif v5 —— 「纸屑场」（2026-08-20 柔和组皮肤重设计，设计源
 * `.issues/2026-08-18-theme-redesign/skins/group4-soft-boards.dc.html` 的
 * `section#g4` campaign 设计表）。
 *
 ...
 *   - **两形制**：圆点（r 2-4.5）与斜方片（8×5 的方片，转 -45°~+45°）。
 ...
 * 精确可表示），seed 仍取板上的 7。形制、尺寸区间、配色轮换、总枚数一处
 * 未改，变的只是随机流本身。
 *
 ...
 *
 * 三个窗按最大外扩 {@link CHIP_REACH}（斜方片半对角 √(4²+2.5²) ≈ 4.72px）
 * 收边，于是「带内、字外」是构造上成立的，不靠抽样运气：
 *   - 顶带 y `[10,28]`，外扩后 y5.28-32.72 < 34
 *   - 左带 x `[20,50]`，外扩后 x15.28-54.72 < 56
 *   - 右带 x `[1230,1262]`，外扩后 x1225.28-1266.72 > 1224，且 < 1280
 *   - 两条侧带 y `[40,655]`，外扩后 y35.28-659.72，上不进右上/左上 logo 带
...
/**
 * 一枚纸屑的最大外扩：斜方片半对角 √(4²+2.5²)，圆点最大 4.5，取大者。
 * 三条带的取样窗按它对实测排字外沿收边（见文件头）。导出给
 * `motif-campaign-motif.test.tsx`——安全区守卫要用同一个数量边界，两处各写
 * 一遍就会各自漂。
 */
export const CHIP_REACH = Math.sqrt(4 * 4 + 2.5 * 2.5)

/**
 * 三条纸屑带的取样窗（顶 / 左 / 右），`n` 是该带的枚数。窗口都已按
 * {@link CHIP_REACH} 对实测排字外沿收过边，推导见文件头。顶带排第一位是
 * 有意的：{@link HALF_FIELD_BANDS} 按下标切半场。
 */
const BANDS: readonly { n: number; x: readonly [number, number]; y: readonly [number, number] }[] = [
  { n: 60, x: [48, 1232], y: [10, 28] },
  { n: 30, x: [20, 50], y: [40, 655] },
  { n: 30, x: [1230, 1262], y: [40, 655] },
]
...
/** 圆点半径区间 [2, 4.5)、斜方片 8×5、转角 ±45°，全部照板。 */
const DOT_R_MIN = 2
const DOT_R_SPAN = 2.5
const CHIP_W = 8
const CHIP_H = 5
const CHIP_ANGLE_SPAN = 90
const CHIP_ANGLE_MIN = -45
const CONFETTI_OPACITY = 0.9

/** 一枚纸屑：圆点或斜方片，`c` 是 `chartPalette` 的下标。 */
type Confetti =
  | { kind: "dot"; band: number; c: number; cx: number; cy: number; r: number }
  | { kind: "chip"; band: number; c: number; cx: number; cy: number; d: string }
...
/** 斜方片：8×5 的方片绕自身中心转 `deg`，四角算成一条闭合 path。
 * 不用 `<rect transform="rotate(...)">`：导出侧 `svg2pptx/dispatch.ts` 明说
 * 「旋转/斜切不在受控子集内」，转角会被当成没转处理。 */
...
const CONFETTI: readonly Confetti[] = (() => {
  ...
  for (let b = 0; b < BANDS.length; b++) {
    ...
    for (let i = 0; i < band.n; i++) {
      const cx = round1(band.x[0] + rnd() * (band.x[1] - band.x[0]))
      const cy = round1(band.y[0] + rnd() * (band.y[1] - band.y[0]))
      const c = k % 4
      if (k % 3 === 2) {
        const deg = Math.round(rnd() * CHIP_ANGLE_SPAN + CHIP_ANGLE_MIN)
        out.push({ kind: "chip", band: b, c, cx, cy, d: chipPath(cx, cy, deg) })
      } else {
        out.push({ kind: "dot", band: b, c, cx, cy, r: round1(DOT_R_MIN + rnd() * DOT_R_SPAN) })
      }
      k++
    }
  }
    ...
    <>
      {CONFETTI.map((piece, i) =>
        halfField && piece.band >= HALF_FIELD_BANDS ? null : piece.kind === "dot" ? (
          ...
          <path key={i} d={piece.d} fill={palette[piece.c]} opacity={CONFETTI_OPACITY} />
        ),
      )}
    </>
  )
