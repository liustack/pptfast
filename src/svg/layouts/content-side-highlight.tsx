 ...
 * expansion, new layout 1 of 3): a genuinely asymmetric two-region page —
 * a standard-width body column (kicker/heading/subheading/`SvgContent`,
 * unchanged geometry from the pool's other single-stack layouts) paired
 * with a *persistent, opaque, self-painted `colors.primary` panel* running
 * the page's full content height on the right edge. The panel is frame, not
 ...
 * layout introduces no width narrower than `audit/capacity.ts` already
 ...
 * layouts: `two-column`/`aside` split the *body's own components* into
 ...
 * this layout's second region is never a component destination at all,
 ...
 * `accessibleInk`-guarded pattern every sibling single-stack layout
...
export function SideHighlightContent({ ir, slide, index, ctx }: SvgTemplateProps) {
  ...
  return (
    <>
      {/* Body column: standard kicker/heading/subheading/content band */}
      {kicker && (
        <text
...
// CONTENT_LAYOUT_DEFS["side-highlight"] entry. Slot `accepts: []` means the slot is not fed by an authored
...
export const layoutDef: LayoutDefinition = {
  ...
  // a persistent, unconditional highlight panel (frame, not a
  ...
  slots: [
    ...
    { name: "body", accepts: "any", capacity: 4 }, // single-stack, 880px column — see registry.ts's CONTENT_LAYOUT_DEFS header for the derivation
    { name: "panel", accepts: [] },
    { name: "meta", accepts: [] },
  ],
  arrangements: "all",
