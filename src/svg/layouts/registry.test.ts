...
describe("capacity metadata: only where the inventory gives hard numbers", () => {
  ...
  it("the remaining 6 content archetypes' body slots carry capacity 4 (W2 task 5 — mirrors the pre-W3 flat single-stack default (4, now DELIVERY_BUDGETS territory), see registry.ts's CONTENT_LAYOUTS header comment)", () => {
    for (const id of Object.keys(CONTENT_ARCHETYPES)) {
