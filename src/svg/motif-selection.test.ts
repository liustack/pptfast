...
describe("resolveMotifId — byte-inertness for the themes this task must not disturb", () => {
  ...
  it("a 1-member candidate set (campaign, ink, crayon, arena, museum, swiss) always resolves to its own anchor regardless of seed or pageKey", () => {
    for (const themeId of ["campaign", "ink", "crayon", "arena", "museum", "swiss"] as const) {
      for (let seed = 0; seed < 20; seed++) {
