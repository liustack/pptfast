...
describe("theme registry v2", () => {
  ...
  // 工业青/机灰。**第四轮评审又把 accent 的炸橘换掉了**：用户 p09/p10
  // 「不要蓝配橙，超级丑」，accent 改成工业蓝 #2F6FBF，本主题从此不出现
  // 暖色。来历逐条见 `themes/enterprise.ts` 的文件头。
  it("enterprise defaults to gallery-white bg + IKB primary + industrial-blue accent + blue/orange chart palette", () => {
    ...
    expect(THEME_STYLES["enterprise"].colors.accent).toBe("#2F6FBF");
