...
describe("registerTheme: unmeasured-font-width console.warn", () => {
  ...
  it("regression: heritage/ink/journal/luxe/memo/museum/runway's heading has no exact table, every builtin's body does — but builtins never call registerTheme, so this never reaches console.warn", () => {
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {})
    const nonExactHeadingBuiltins = new Set(["heritage", "ink", "journal", "luxe", "memo", "museum", "runway"])
    for (const id of CANONICAL_THEME_IDS) {
