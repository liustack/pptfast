...
describe("cross-theme layout divergence (the plan's core defect)", () => {
  it("resolves NOT-all-identical layout sequences across the 18 canonical themes for a fixed IR + fixed seed", () => {
...
describe("allocation wave drift: the cover slot moved for eight themes and nothing else moved for anyone", () => {
  it("the pre-allocation fixture covers the 17 themes that existed at capture (arena landed later)", () => {
    expect(Object.keys(preAllocationFixture).sort()).toEqual(
      CANONICAL_THEME_IDS.filter((id) => id !== "arena").sort(),
    )
    for (const themeId of CANONICAL_THEME_IDS.filter((id) => id !== "arena")) {
      expect(Object.keys(preAllocationFixture[themeId]!).sort(), themeId).toEqual(["1", "2", "3", "4", "5"])
    }
  })

  it("no page other than the cover moved, for any of the 17 themes captured at the time — zero content displaced outside the slot this wave declares", () => {
    for (const themeId of CANONICAL_THEME_IDS.filter((id) => id !== "arena")) {
      for (const seed of FIXTURE_SEEDS) {
  ...
  it("the seven themes neither edit re-weighted keep their exact pre-wave cover picks", () => {
    const untouched = CANONICAL_THEME_IDS.filter((id) => ALLOCATION_COVER_MOVES[id] === undefined && id !== "arena")
    ...
    // fixture captured (arena landed later and is out of this wave's scope).
...
describe("forced theme-tendency × stress-content geometry audit (closes the T2 review's coverage gap)", () => {
  ...
  it("sanity: 56 declared theme×layout combinations exist to force-audit — every id every theme leans toward on cover/chapter/ending, rendered with pathological content", () => {
    ...
    // to two, while vermilion's stayed at two with one swapped. arena
    // (2026-08-21) adds two cover ids and nothing on chapter/ending. The
